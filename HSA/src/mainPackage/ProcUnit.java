...
public class ProcUnit extends Thread
{
	...
	public void run()
	{
		...
		while (true)
		{
			...
			if (!incTuples.isEmpty() && (myRank == coreCount - 1 || rightNeighbour.approxIncTuplesSize.get() < inputTupleThreshold))
			{
				...
				for (int opIndex = 0; opIndex < operationList.size(); opIndex++)	//Iterate through all Operations, that are being executed currently
				{
					...
					for (long curWindowEnd = firstWindowEnd; curWindowEnd <= lastWindowEnd; curWindowEnd += windowStep)	//Iterate through all Windows the current tuple might contribute to
					{
					    ...
					    long tempHash = MurmurHash.hash(bytes, 1);
						if ( (tempHash % coreCount + coreCount) % coreCount == myRank)
			...
			if (!results.isEmpty())
			{
				...
				{
					System.out.println("Tuple time: " + (nanotimeTuples/1000000) + ". Result Set time: " + (nanotimeResultSets/1000000) + ". ");
				}
	...
	private void checkIfToCreateNewResultSet(Tuple curTuple)
	{
		...
		{
			ResultSet newResultSet = new ResultSet(curTuple.time, curTuple.realtime); //was curTuple.time-1 once, but since upper limit it excluding we don't need -1 i think
...
//bytes[0] = (byte)(tempHashSum & 0xffL);
//bytes[1] = (byte)((tempHashSum & 0xff00L) >> 8);
//bytes[2] = (byte)((tempHashSum & 0xff0000L) >> 16);
//bytes[3] = (byte)((tempHashSum & 0xff000000L) >> 24);
//bytes[4] = (byte)((tempHashSum & 0xff00000000L) >> 32);
//bytes[5] = (byte)((tempHashSum & 0xff0000000000L) >> 40);
//bytes[6] = (byte)((tempHashSum & 0xff000000000000L) >> 48);
//bytes[7] = (byte)((tempHashSum & 0xff00000000000000L) >> 56);
