	...
	public void run()
	{
		...
		while (true)
		{
			...
			if (curIncTuple != null || curOwnTuple != null)
			{
				...
				{
					if (curIncTuple == null || curIncTuple.time >= curOwnTuple.time)		//In case we take the tuple from the incOwnTuples-Queue, we have to calculate the hash functions
					{
						...
						for (int opIndex = 0; opIndex < operationList.size(); opIndex++)	//Iterate through all Operations, that are being executed currently
						{
							...
							int groupByHashCode = curTuple.sData.hashCode();
							curTuple.hashStringList.add(hashString);
							...
							ArrayList<LinkedList<Long>> responsibilityList = new ArrayList<LinkedList<Long>>();
							curTuple.responsibilityListList.add(responsibilityList);
							for (int i = 0; i < coreCount; i++)
							{
								responsibilityList.add(new LinkedList<Long>());
							}
							
							//Execute hash-function to distribute windows evenly to the PUs; store within the current tuple the responsible PU for all Windows to which it contributes
							for (long curWindowEnd = firstWindowEnd; curWindowEnd <= lastWindowEnd; curWindowEnd += windowStep)
							{
								long tempHashSum = curWindowEnd+operationList.get(opIndex).operationId+groupByHashCode;
								byte[] bytes = ByteBuffer.allocate(8).putLong(tempHashSum).array();
							    int tempHash = (MurmurHash.hash(bytes, 1) % coreCount + coreCount) % coreCount;
							    responsibilityList.get(tempHash).add(curWindowEnd);
							}
						}
					}
					else
				...
				if (curTuple != null)													
				{
					...
					//Iterate through all responsibilityListLists, take own responsibilityList and handle windows
					LinkedList<Long> curWindowList;
					for (int opIndex = 0; opIndex < operationList.size(); opIndex++)				//Iterate through all Operations, that are being executed currently
					{
						curWindowList = curTuple.responsibilityListList.get(opIndex).get(myRank);
						String hashString = curTuple.hashStringList.get(opIndex);
						while (!curWindowList.isEmpty())											//Iterate through all Windows the current PU has to take care of
						{
							//----------FindOrCreateWindow part: Find the necessary window or create it if not existent yet----------//
							long curWindowEnd = curWindowList.removeFirst();
							...
							if (curHashMap != null)
							{
								...
								if (curWindow == null)
								{
									curWindow = Window.CreateWindow(curWindowEnd, operationList.get(opIndex), curTuple.sData);
...
/*
old hashfunction
//responsibilityList.get((((Long)(curWindowEnd+operationList.get(opIndex).operationId+groupByHashCode)).toString().hashCode() % coreCount + coreCount) % coreCount).add(curWindowEnd);
 * 
